...
| INV-10 | **Approvals are one-shot, expiring, digest-bound.** Never bound to an agent, a session, or an action type. A missing expiry is treated as expired, not as permanent. | B2, B3 | `tests/approval/test_maker_checker.py` |
