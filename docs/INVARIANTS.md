...
| INV-8 | **A rail adapter cannot change the decision layer.** Adding a rail is a new closed payload variant and a new adapter. If it requires touching decision code, the abstraction is wrong. | A6 | `lint-imports`; falsified by `tests/adapters/test_falsification.py` |
