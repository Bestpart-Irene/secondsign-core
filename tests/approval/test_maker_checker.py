...
"""Unit tests for the digest-bound maker-checker flow.

The properties this enforces are the ones a replay or a self-approval would
break: an approval binds to exactly one digest (B2), is one-shot, expires with a
...
def test_request_produces_an_approval_bound_to_the_decision_digest():
    decision = make_review_decision()
    pending = fresh_maker_checker().request(
        decision, MAKER, approval_id="a1", expires_at=EXPIRES_AT
    )
    assert pending.digest == decision.digest
...
def test_only_a_review_decision_can_be_requested():
    ...
    with pytest.raises(ValueError, match="REVIEW"):
        fresh_maker_checker().request(denied, MAKER, approval_id="a1", expires_at=EXPIRES_AT)
...
def test_a_valid_consume_grants():
    ...
    assert grant.digest == pending.digest
...
def test_a_verdict_for_a_different_digest_is_rejected():
    ...
    mc = fresh_maker_checker()
    pending = make_pending()
    other_digest = IntentDigest(value="f" * 64)
    verdict = CheckerVerdict(checker=CHECKER, digest=other_digest, approved=True)
...
def test_the_maker_cannot_be_the_checker():
    ...
    verdict = CheckerVerdict(checker=self_checker, digest=pending.digest, approved=True)
...
def test_a_rejected_verdict_does_not_grant():
    mc = fresh_maker_checker()
    pending = make_pending()
    verdict = CheckerVerdict(checker=CHECKER, digest=pending.digest, approved=False)
...
def test_a_failed_consume_does_not_burn_the_approval():
    ...
    mc = fresh_maker_checker()
    pending = make_pending()
    bad = CheckerVerdict(checker=CHECKER, digest=IntentDigest(value="0" * 64), approved=True)
...
def test_approvals_are_not_bound_to_an_agent_or_action_type():
    """The forbidden coupling: a PendingApproval carries a digest, a maker and a
